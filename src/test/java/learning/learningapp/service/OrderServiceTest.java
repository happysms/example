...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception {
        ...
        member.setName("회원1");
        member.setAddress(new Address("서울", "강", "123-123"));
        em.persist(member);
        ...
        Assert.assertEquals("주문한 상품 종류가 수가 정확해야 한다. ", 1, getOrder.getOrderItems().size());
        Assert.assertEquals("주문 가격은 가격 * 수량이다", 10000 * orderCount, getOrder.getTotalPrice());
        Assert.assertEquals("주문 수량만큼 재곡 줄어야 한다.", 8, book.getStockQuantity());
    }
