...
import learning.learningapp.domain.*;
 ...
 * SPRING2 BOOK
 */
@Component  //스프링에 컴포넌트 스캔 대상이된다.
@RequiredArgsConstructor
public class initDB {

    private final InitService initService;

    @PostConstruct  // 스프링 빈이 모두 올라오고 나면 바로 호출해준다.  , 스프링 라이프사이클이 있기에 별도의 InitService 빈을 만들어서 실행을 해주어야한다.
    public void init(){  //애플리케이션 로딩시점에 바로 실
        initService.dbInit1();
        initService.dbInit2();
    }
